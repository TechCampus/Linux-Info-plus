...
while true; do
    echo "$(tput setaf 2)1. معلومات النظام"
    echo "2. معلومات الذاكرة العشوائية/المعالج"
    echo "3. المساحة التخزينية"
    echo "4. المستخدمون"
    echo "5. الذين يستخدمون النظام حالياً"
    echo "6. الخدمات الجارية"
    echo "7. البورتات المفتوحة ومعلومات النظام الأساسية"
    echo "8. فحص الشبكة المحلية"
    echo "9. إعدادات DNS"
    echo "10. مهام Cron"
    echo "11. اختبار سرعة الإنترنت للخادم"
    echo "12. البرامج المثبتة"
    echo "13. تحليل صغير لسجل Apache"
    echo "Y. تثبيت البرامج الضرورية لهذه الأداة"
    echo
    echo -n "$(tput setaf 3)أدخل اختيارك، أو 0 للخروج: " ;tput sgr0
    read choice
    echo
...
case $choice in
     1)
            # عرض الرأس
            echo "$(tput setaf 1)====================== معلومات النظام =============================";tput sgr0

            # عرض اسم المضيف
            echo -e "$(tput setaf 4)اسم المضيف:\t\t$(tput setaf 6)"`hostname`;tput sgr0

            # عرض المستخدم الحالي
            echo -e "$(tput setaf 4)المستخدم:\t\t\t$(tput setaf 6)"`whoami`;tput sgr0

            # عرض وقت تشغيل النظام
            echo -e "$(tput setaf 4)وقت التشغيل:\t\t\t$(tput setaf 6)"`uptime | awk '{print $3,$4}' | sed 's/,//'`;tput sgr0

            # عرض الشركة المصنعة للنظام
            echo -e "$(tput setaf 4)الشركة المصنعة:\t\t$(tput setaf 6)"`cat /sys/class/dmi/id/chassis_vendor`;tput sgr0

            # عرض اسم المنتج الخاص بالنظام
            echo -e "$(tput setaf 4)اسم المنتج:\t\t$(tput setaf 6)"`cat /sys/class/dmi/id/product_name`;tput sgr0

            # عرض الرقم التسلسلي للنظام
            echo -e "$(tput setaf 4)الرقم التسلسلي:\t\t$(tput setaf 6)"`cat /sys/class/dmi/id/product_serial`;tput sgr0

            # عرض نوع الجهاز (افتراضي أو مادي)
            echo -e "$(tput setaf 4)نوع الجهاز:\t\t$(tput setaf 6)"`vserver=$(lscpu | grep Hypervisor | wc -l); if [ $vserver -gt 0 ]; then echo "VM"; else echo "Physical"; fi`;tput sgr0

            # عرض اسم نظام التشغيل وإصداره
            echo -e "$(tput setaf 4)نظام التشغيل:\t\t$(tput setaf 6)"`hostnamectl | grep "Operating System" | cut -d ' ' -f5-`;tput sgr0

            # عرض إصدار نواة النظام
            echo -e "$(tput setaf 4)النواة:\t\t\t$(tput setaf 6)"`uname -r`;tput sgr0

            # عرض بنية النظام
            echo -e "$(tput setaf 4)الهندسة المعمارية:\t\t$(tput setaf 6)"`arch`;tput sgr0

            # عرض اسم المعالج
            echo -e "$(tput setaf 4)اسم المعالج:\t\t$(tput setaf 6)"`awk -F':' '/^model name/ {print $2}' /proc/cpuinfo | uniq | sed -e 's/^[ \t]*//'`;tput sgr0

            # عرض المستخدمين النشطين
            echo -e "$(tput setaf 4)المستخدم النشط:\t\t$(tput setaf 6)"`w | cut -d ' ' -f1 | grep -v USER | xargs -n1`;tput sgr0

            # عرض عنوان IP الرئيسي للنظام
            echo -e "$(tput setaf 4)العنوان الرئيسي للنظام:\t\t$(tput setaf 6)"`hostname -I`;tput sgr0
...
;;
     2)
            # يعرض العنوان
            echo "$(tput setaf 1)====================== معلومات الذاكرة العشوائية/المعالج =============================";tput sgr0
...
;;
     3)
            # يعرض العنوان
            echo "$(tput setaf 1)====================== المساحة التخزينية =============================";tput sgr0
...
;;
     4)
            # يعرض العنوان
            echo "$(tput setaf 1)====================== المستخدمون =============================";tput sgr0
     ...
     5)
            ...
            # يعرض العنوان
            echo "$(tput setaf 1)====================== من متصل؟ ==========================";tput sgr0
     ...
     7)
            ...
            spinner()
            {
                ...
                while [ "$(ps a | awk '{print $1}' | grep $pid)" ]; do
                    local temp=${spinstr#?}
                    printf "\r $(tput setaf 4)جارٍ الفحص ..............$(tput sgr0) [%c] " "$spinstr"
                    local spinstr=$temp${spinstr%"$temp"}
                    sleep $delay
                done
                printf "\r $(tput setaf 4)تم الانتهاء من الفحص!         $(tput sgr0)"
                echo
            }
            ...
            if sudo -v >/dev/null 2>&1; then
                ...
                printf "\n\033[31mنتائج تحديد نوع النظام التشغيل:\033[0m\n"
                echo "===================================="
                sudo nmap -O --open localhost | awk '/Running/{print $2,$3,$4,$5,$6,$7,$8,$9;}/OS details/{print $0;}'
                echo
            else
                echo "$(tput setaf 1)يتطلب تشغيل الفحص كلمة مرور sudo.$(tput sgr0)"
                sudo true
            fi
            echo
;;
     8)
            ...
            echo "$(tput setaf 1)جارٍ الفحص ...";tput sgr0
            ...
            echo -e "\033[31mنتائج الفحص المحلي للآي بي:\033[0m"
            echo -e "\033[34m======================\033[0m"
            echo -e "\033[34mعنوان IP   |  عنوان MAC\033[0m"
            echo -e "\033[34m-------------|------------------\033[0m"
...
;;
        11)
                ...
                echo -n "$(tput setaf 6)جاري التحميل"
...
;;
        12)
            ...
            while true; do
                echo
                echo "$(tput setaf 2)اختر خيارًا:";tput sgr0 # طباعة نص القائمة باللون الأخضر
                echo "$(tput setaf 1)A) عرض جميع البرامج المثبتة" # طباعة الخيار أ باللون الأحمر
                echo "B) البحث عن برنامج مثبت" # طباعة الخيار ب
                echo "C) عرض جميع البرامج العاملة" # طباعة الخيار ج
                echo "$(tput setaf 7)D) العودة";tput sgr0 # طباعة الخيار د باللون الأبيض وإعادة تعيين لون النص
                read option # قراءة إدخال المستخدم وتخزينه في المتغير option

                case "$option" in
                    "A") # إذا اختار المستخدم الخيار أ، سيتم عرض جميع البرامج المثبتة
                        echo "البرامج المثبتة:"
                ...
                "B") # إذا اختار المستخدم الخيار ب، سيتم البحث عن البرامج المثبتة
                            echo "أدخل مصطلح البحث:"
                            read search_term # طلب من المستخدم إدخال مصطلح البحث
                            echo "النص المبحوث عنه: $(tput setaf 2)"$search_term ;tput sgr0 # طباعة مصطلح البحث باللون الأخضر
                            sleep 2 # إيقاف التشغيل لمدة 2 ثوانٍ
                            echo "البرامج المطابقة:"
                        ...
                        *) # إذا أدخل المستخدم خيارًا غير صالح، سيتم طباعة رسالة خطأ
                            echo "خيار غير صالح. يرجى إدخال A أو B أو C."
                            ;;
                    esac
                done
...
;;
        13)
            ...
            echo -e "\033[31mتحليل سجل الزيارات Apache:\033[0m" # طباعة العنوان الرئيسي باللون الأحمر
        ...
        Y) # إذا أدخل المستخدم "Y"، تحقق من وجود الحزم المفقودة وقم بتثبيتها إذا لزم الأمر
            ...
            if [[ ${#missing_packages[@]} -eq 0 ]]; then
                echo
                echo "$(tput setaf 1)جميع الحزم اللازمة مثبتة بالفعل.";tput sgr0
                echo
            else # وإلا، اطلب من المستخدم تثبيت الحزم المفقودة
                echo "الحزم التالية مطلوبة: ${missing_packages[@]}"
                read -p "هل تريد تثبيت هذه الحزم؟ (y/n) " answer
                echo
                if [[ "$answer" == "y" ]]; then # إذا أدخل المستخدم "y"، قم بتثبيت الحزم المفقودة باستخدام apt-get
                sudo apt-get install "${missing_packages[@]}"
                echo
                echo "$(tput setaf 1)تم تثبيت الحزم اللازمة!";tput sgr0
                echo
                else # وإلا، أعلم المستخدم بأن الحزم اللازمة غير مثبتة
                echo
                echo "$(tput setaf 1)الحزم اللازمة غير مثبتة.";tput sgr0
                echo
                fi
            fi
     ;;
     0)
            ...
            " | lolcat
            echo "$(tput setaf 2)أتمنى لك يومًا سعيدًا :)";tput sgr0
     break
     ;;
     *)
            echo "خطأ، يرجى اختيار من 1-13، أو اختر 0 للخروج"
     ;;
esac  
done